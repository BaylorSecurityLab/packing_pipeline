...
label_one() {  # <nas_dir> <testcase> <family> <version>
  ...
  git add -f "manifest/empirical_types_${tag}.yaml" docs/EMPIRICAL_TYPE_LABELS.md 2>/dev/null
  git add "empirical_results/qemu_runtime/configs/${tag}.json" 2>/dev/null || true
  git commit -q -m "Empirical label: $3 $4 $2 (see docs/EMPIRICAL_TYPE_LABELS.md)
