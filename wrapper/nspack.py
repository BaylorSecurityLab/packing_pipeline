...
class NSpack(BaseGUI):
    """
    Wrapper for NSpack GUI automation.

    1. Launch nSpack.exe
    2. Find main window
    3. Click Open button (0.9, 0.95) → Windows Explorer file picker
    4. Select input file
    5. Click Compress button (0.1, 0.95)
    ...
    def _find_main_window(self, timeout=15):
        """
        Scan for any visible non-excluded window (NSpack may not share PID with launcher).
        """
        print("\n[INFO] Waiting for NSpack main window...")
        ...
        print("[ERROR] NSpack main window not found within timeout")
        return False

    def wait_for_packing_complete(self, input_file_path):
        """
        Watch the input file for lock release after NSpack finishes writing.
    ...
    def run(self, click_mode="all", file_path=None, output_dir=None) -> bool:
        """
        Main execution flow for NSpack.
        ...
        try:
            ...
            # Step 1: Launch NSpack
            print("\n[INFO] Launching NSpack...")
            if not self.launch_application():
                print("[ERROR] Failed to launch application")
                return False

            # Step 2: Find main window
            if not self._find_main_window(timeout=self.LONG_TIMEOUT):
                print("[ERROR] Could not find NSpack main window")
                self.close_application()
                return False

            self.center_window_on_monitor(monitor_number=0)
            print(f"[INFO] NSpack window found: '{self.window.title}'")
            time.sleep(0.5)

            # Step 3: Click Open button → launches Windows Explorer file picker
            self.click_at_percent(0.9, 0.95, "Open button")
            time.sleep(1)

            # Step 4: Select file via picker (direct keystrokes — no title matching needed)
            print(f"[INFO] Navigating to directory: {input_file.parent}")
            pyautogui.hotkey("ctrl", "l")
            time.sleep(0.3)
            pyperclip.copy(str(input_file.parent))
            pyautogui.hotkey("ctrl", "v")
            time.sleep(0.3)
            pyautogui.press("enter")
            time.sleep(1)

            print(f"[INFO] Entering filename: {input_file.name}")
            pyautogui.hotkey("alt", "n")
            time.sleep(0.3)
            pyperclip.copy(input_file.name)
            pyautogui.hotkey("ctrl", "v")
            time.sleep(0.3)
            pyautogui.press("enter")
            time.sleep(1)
            ...
            if packed_file:
                ...
                if final_path:
                    ...
                    print(f"{'=' * 60}")
                self.close_application()
                print("\n[SUCCESS] NSpack automation complete!")
                return True
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="NSpack GUI Automation Wrapper")
    parser.add_argument(
    ...
    wrapper = NSpack(yaml_path, main_dir)
