"""
WinUpack GUI Automation Wrapper - Using Base GUI Wrapper
"""
...
class WinUpack(BaseGUI):
    """
    Wrapper for WinUpack 0.39 GUI automation using the BaseGUIWrapper.

    WinUpack is a GUI frontend for the Upack PE compressor.
    Output behavior is in-place (modifies the input file directly).
    """

    def __init__(self, yaml_path, main_dir):
        """Initialize WinUpack wrapper"""
    ...
    def wait_for_packing_complete(self, input_file_path):
        ...
        WinUpack modifies the input file in-place, so we watch the input
        file for lock release. Requires N consecutive unlocked checks.

        Args:
            input_file_path: Path to the file being packed (in-place)
    ...
    def run(self, click_mode="all", file_path=None, output_dir=None):
        """
        Main execution flow for WinUpack.

        Opens the application, pastes the input file path, tabs 12 times
        to reach the Pack button, triggers packing, and watches the file.

        Args:
            click_mode: Configuration mode (unused for now)
        ...
        try:
            ...
            if not self.find_window():
                ...
                if not self.find_window(window_title="upack"):
                    print("[ERROR] Could not find WinUpack window")
                    return False

            print("\n[SUCCESS] WinUpack launched successfully!")
            ...
            print("\n[SUCCESS] WinUpack opened and centered on monitor 1!")

            # Step 5: Paste the input file path via Ctrl+V
            if file_path:
            ...
            # Step 6: Tab 12 times to reach the Pack button
            print("\n[INFO] Tabbing 12 times to reach Pack button...")
            for _ in range(12):
                pyautogui.press("tab")
                time.sleep(0.1)
            print("[SUCCESS] Reached Pack button!")

            # Step 7: Press Enter to trigger packing
            time.sleep(0.2)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="WinUpack GUI Automation Wrapper",
    ...
    wrapper = WinUpack(yaml_path, main_dir)
